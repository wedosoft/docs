...
:::info 문서 목적
이 문서는 "서비스 데스크 상태(Service Desk Statuses)" 사용자 정의를 통해 조직의 워크플로우에 맞는 티켓 상태를 구성하는 방법을 안내해요.
:::

## 개요

Freshservice는 기본적으로 4가지 티켓 상태를 제공하지만, 조직의 특별한 워크플로우 요구사항에 맞춰 커스텀 상태를 추가할 수 있어요. 커스텀 상태를 통해 에이전트와 고객에게 서로 다른 메시지를 표시하고, SLA 타이머를 세밀하게 제어할 수 있어요.
...
### 조직별 워크플로우 반영
조직의 특별한 업무 프로세스를 티켓 상태에 반영할 수 있어요:
...
### 이중 상태 표시 기능
에이전트와 고객에게 서로 다른 메시지를 표시할 수 있어요:
...
:::info 워크플로우 실행 순서
워크스페이스 관리자는 글로벌 및 로컬 워크플로우 목록을 모두 볼 수 있어요. **글로벌 워크플로우가 먼저 실행**되고, 그 다음 워크스페이스 레벨 워크플로우가 실행되어 우선순위를 가져요.
:::

### 2단계: 상태 필드 접근

1. **Ticket Fields** 섹션을 클릭해서 확장해요
2. 티켓 폼에서 아래로 스크롤해서 **Status** 필드를 찾아요
3. **Status** 필드를 클릭해요
...
1. **Dropdown Choices** 섹션에서 현재 상태 목록을 확인해요
2. 하단의 **+Add item** 버튼을 클릭해요
3. 새 상태에 대한 정보를 입력해요:
   - **에이전트용 이름**: 내부 에이전트가 보는 상태명
   - **고객용 이름**: 고객 포털에서 표시되는 상태명
4. **Save** 버튼을 클릭해서 저장해요
...
:::tip 시스템 위젯 연동
"On-Hold" 위젯은 **기본 시스템 위젯**으로 "Tickets on Hold" 필터와 연동돼요. 대시보드에서 위젯을 제거하면 티켓 목록뷰의 해당 필터도 함께 삭제돼요.
:::
