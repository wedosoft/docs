...
Freshservice 서비스 카탈로그는 조직 내 IT 서비스와 요청을 체계적으로 관리할 수 있는 중앙 집중식 플랫폼으로, 사용자가 요청할 수 있는 모든 IT 서비스를 구조화된 방식으로 제시하는 디지털 메뉴입니다.

:::info 서비스 카탈로그 핵심 구성요소
...
### 상황 1: 대기업 IT 부서의 소프트웨어 요청 체계화
**목표**: 600명 규모 조직에서 소프트웨어 라이선스 요청을 체계적으로 관리

**방법**:
1. **소프트웨어 카테고리 생성**: "Software & Applications" 카테고리 하위에 "Creative Tools", "Development Tools", "Business Applications" 세분화
2. **승인 워크플로우 설정**: 
   - 1-10만원: 팀장 승인
   - 10-50만원: 부서장 승인  
   - 50만원 초과: IT 예산 위원회 승인
3. **커스텀 필드 추가**: 사용 목적, 예상 사용자 수, 라이선스 유형, 비용 센터 정보

**결과**: 소프트웨어 요청 처리 시간 40% 단축, 승인 프로세스 투명성 확보

### 상황 2: 제조업체의 하드웨어 교체 프로세스 자동화
**목표**: 노후 PC/노트북 교체 요청을 효율적으로 처리

**방법**:
1. **하드웨어 교체 서비스 아이템 생성**: 현재 장비 정보, 사용 용도, 성능 요구사항 필드 추가
2. **자산 관리 연동**: 요청 시 기존 자산 정보 자동 조회
3. **배송/설치 일정 관리**: 납품업체와 연동하여 배송 일정 자동 업데이트

**결과**: 하드웨어 교체 요청 처리 시간 60% 단축, 자산 관리 정확도 95% 향상

:::success 서비스 카탈로그 운영 성공 팁
