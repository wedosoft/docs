...
Freshservice의 기본 사용자 필드는 상담원과 요청자에 대한 많은 정보를 저장할 수 있어요. 하지만 조직에 특화된 추가 정보가 필요한 경우가 있어요. 예를 들어, 각 사용자의 사번, 워크스테이션 번호, 비용 센터 등을 저장하고 싶을 수 있어요.

**사용자 필드(User Fields)** 기능을 사용하면 상담원과 요청자 모두에게 적용되는 맞춤 필드를 생성할 수 있어요. 요청자 필드를 기반으로 구성된 워크플로가 상담원이 요청자로 설정되었을 때도 동일하게 작동해요.

## 맞춤 필드 유형

다음과 같은 맞춤 필드 유형을 추가할 수 있어요:
...
2. **관리자 → 사용자 관리 → 사용자 필드**로 이동해요.
3. 계정에 여러 워크스페이스가 있는 경우:
   - **관리자 → 글로벌 설정 → 사용자 관리 → 사용자 필드**로 이동해요.
...
1. **요청자 필드** 또는 **상담원 필드** 탭을 선택해요.
2. 원하는 맞춤 필드 유형(예: 날짜)을 메뉴에서 클릭해서 양식에 추가해요.
3. 필요에 따라 필드 순서를 재정렬할 수 있어요.
...
양식이 원하는 대로 구성되었다면 **저장**을 클릭해요.

## 상담원과 요청자별 개별 설정

동일한 필드에 대해 상담원과 요청자별로 다른 동작을 구성할 수 있어요:
...
### 1. 조건부 필드 표시
특정 조건에서만 필드가 표시되도록 설정할 수 있어요:
- 요청자 유형에 따른 맞춤 필드 표시
- 부서별로 다른 필드 세트 적용

### 2. API 연동 활용
맞춤 필드는 Freshservice API를 통해서도 접근 가능해요:
```json
{
...
- 반대의 경우도 마찬가지입니다
- 각 필드의 설정(필수 여부, 편집 가능 여부)은 개별적으로 구성할 수 있어요
:::
